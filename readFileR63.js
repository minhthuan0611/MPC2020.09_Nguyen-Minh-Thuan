...
async function giainen() {
    if (isFileExist(dir + '/sbv_20200328.zip') == true) {
        await decompress(dir + '/sbv_20200328.zip', dir).then(i => { fs.unlinkSync(dir + '/sbv_20200328.zip') })
    }
    if (isFileExist(dir + '/report_20200328.tar') == true) {
        await decompress(dir + '/report_20200328.tar', dir + '/report_20200328')
            .then(i => { fs.unlinkSync(dir + '/report_20200328.tar') })
    }
    if (isFileExist(dir + '/report_20200328.tar.gz') == true) {
        await decompress(dir + '/report_20200328.tar.gz', dir + '/report_20200328')
            .then(i => { fs.unlinkSync(dir + '/report_20200328.tar.gz') })
    }
}
...
giainen().then(i => { importData() })
    //importFile('20200928/report_20200328/R63_Trial_Balance_20200328', 'R63_Trial_Balance_20200328');
...
async function importData() {
    ...
    console.log('Kiểm tra file có tồn tại');
    let currentStartTime = moment();
    while (isFileExist(cardInfoFileName) == false && currentStartTime.isBefore(endCronTime)) {
        await giainen()
        console.log("Thời gian hiện hành: " + currentStartTime);
        console.log(`Không đủ file ${cardInfoFileName} trong dir: ` + dir);
    ...
    if (isFileExist(cardInfoFileName)) {

        runningTime = 0;
        detailStart = moment().toString();
        while (runningTime++ < MAX_RETRY) {
            console.log("========================================================");
            console.log("Starting import SBV File - Time : " + runningTime);
            try {
                ...
                console.log(`Importing SBV FAILED ${ex}`);
    ...
    while (isFileExist(dailyTransactionFileName) == false && currentStartTime.isBefore(endCronTime)) {
        await giainen()
        console.log("Thời gian hiện hành: " + currentStartTime);
        console.log(`Không đủ file ${dailyTransactionFileName} trong dir: ` + dir);
        console.log(`Dừng ${SLEEP_MINUTES} phút trước khi thử lại...`);
        await sleep(0.1 * 60 * 1000);
        currentStartTime = moment();
        console.log('Thử kiểm tra lại ...');
    }
    if (isFileExist(dailyTransactionFileName)) {

        runningTime = 0;
        detailStart = moment().toString();
        while (runningTime++ < MAX_RETRY) {
            console.log("========================================================");
            console.log("Starting import R02 File - Time : " + runningTime);
            try {
                ...
                console.log("Import R02 FAILED", e);
                if (runningTime < MAX_RETRY) {
                    console.log("Retrying ...");
                    await sleep(10 * 1000);
                } else {
     ...
     */
    while (isFileExist(balanceName) == false && currentStartTime.isBefore(endCronTime)) {
        await giainen()
        console.log("Thời gian hiện hành: " + currentStartTime);
        console.log(`Không đủ file ${balanceName} trong dir: ` + dir);
        console.log(`Dừng ${SLEEP_MINUTES} phút trước khi thử lại...`);
        await sleep(0.1 * 60 * 1000);
        currentStartTime = moment();
        console.log('Thử kiểm tra lại ...');
    }
    if (isFileExist(balanceName) == true) {

        runningTime = 0;
        detailStart = moment().toString();
        while (runningTime++ < MAX_RETRY) {
            console.log("========================================================");
            console.log("Starting import R63 File - Time : " + runningTime);
            try {
                ...
                console.log("Import R63 FAILED", e);
...
async function handleFinish() {
    ...
    dataImportHistory.startTime = startCronTime1;
